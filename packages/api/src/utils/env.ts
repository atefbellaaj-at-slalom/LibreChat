...
export function processMCPEnv(params: {
  ...
  if ('apiKey' in newObj && newObj.apiKey) {
    ...
    if (apiKeyConfig.source === 'admin' && apiKeyConfig.key) {
      ...
      // Initialize headers if needed and add the API key header
      if (!('headers' in newObj)) {
        (newObj as { headers?: Record<string, string> }).headers = {};
      }
      (newObj as { headers: Record<string, string> }).headers[headerName] = headerValue;
    }
  }
